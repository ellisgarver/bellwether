...
- **PIIE via Wayback CDX enumeration (ADR-026, 2026-06-04).** PIIE's Drupal sitemap lists only the 2016+ `/YYYY/` URLs, structurally missing ~889 pre-2016 flat-slug publications and most blog history (silent under-capture: 857 records, no 2010-2015). `PIIEIngestor` now enumerates the full publication+blog URL set from Wayback CDX (`_cdx_enumerate`/`_cdx_query`/`_cdx_get`, fail-loud) unioned with the sitemap walk (freshness only), deduped; bodies still fetched live via curl_cffi (`_piie_get`); page date is authoritative (slug year never trusted). Covers both trade-blog slug eras + china-economic-watch. Budget bumped 3h→6h.
