...
### Post-patch fixes (2026-05-18, pre full re-ingest)

1. **Beige Book historical (`src/mnd/ingestion/fed.py`):** added archive-page walk and dual URL-pattern enumeration — historical issues live at `/monetarypolicy/beigebook/beigebook{YYYYMM}.htm` (extra subdirectory) while modern issues at `/monetarypolicy/beigebook{YYYYMM}.htm`. Earlier enum tried only the modern path.
...
3. **fed_chicago (`src/mnd/ingestion/institutional.py`):** sitemap walk now matches eight Chicago Fed publication series (chicago-fed-letter, economic-perspectives, working-papers, policy-discussion-papers, public-policy-papers, profitwise, insights, insights-blog) instead of only chicago-fed-letter.
4. **fed_atlanta (`src/mnd/ingestion/institutional.py`):** new sitemap walker using `_atlanta_get()` (curl_cffi with Chrome impersonation) covers working papers, policy hub, macroblog, and economy matters; falls back to RSS only if the sitemap is unreachable. Prior path used `_parse_rss` directly and was 403'd by atlantafed.org's bot filter.
...
1. **Full re-ingest (NUKE_RAW=1) with all post-patch fixes baked in.** This is the methodology-lock-in re-ingest: ADR-015 canonical filter + four ingestor coverage fixes (Beige Book historical, FEDS Notes legacy, fed_chicago multi-series, fed_atlanta curl_cffi+sitemap). Submit `scripts/rcc/submit_full_pipeline.sh` with `NUKE_RAW=1` after the in-flight patch ingest (49771521) finishes.
2. **Post-re-ingest validation:** filter → embed → cluster → validate. Expect ≥8/10 anchor recovery given the JEL audit additions + corpus coverage closures. If achieved, tick Phase 2 and Phase 3 boxes.
3. **PIIE undercapture.** 179 vs ~800+ expected — likely `article.teaser` selector stale after PIIE redesign. Now also benefits from canonical filter application per ADR-015 step 4.
4. **Phase 4** — pre-registration finalize + full anchor validation (including 6 fizzled anchors). Cite JEL methodology block + ADR-015. Blocked by items 1-2.
