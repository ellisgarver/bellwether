...
- **CBO via Wayback Machine CDX (ADR-021, 2026-05-21).** Supersedes ADR-017's Playwright+curl_cffi hybrid. DataDome tightened to the point that even headless Chromium inside Playwright can't resolve the JS challenge — it serves the interstitial without ever clearing it, and the "clearance" cookie is a rotating challenge-stub. `CBOIngestor` now enumerates cbo.gov publication URLs via `web.archive.org/cdx/search/cdx` (year-sharded, 0.5s politeness) and fetches snapshot bodies via `web.archive.org/web/{ts}id_/{url}` (`id_` = raw archive, no toolbar). The canonical Article.url is still cbo.gov — Wayback is just the access path. Playwright is no longer load-bearing; kept in `requirements.txt` in case DataDome relaxes.
