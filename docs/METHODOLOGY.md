...
The parametric fits use Bayesian inference (PyMC). Every reported number is either a direct measurement off the observed curve (rise/decay rate, peak, plateau, total reach) or, for Bass, a coefficient with a field-anchored prior (the Sultan–Farley–Lehmann 1990 meta-analysis of 213 diffusion studies). No number rests on the unobservable "population" the narrative could have reached or on borrowed epidemiological constants: an earlier draft carried disease-literature LogNormal priors on the SIR β/γ and reported an R₀ = β/γ posterior, but R₀ is not identifiable from a single attention curve — its priors, not the data, set the number — so R₀ was dropped and the SIR priors are now data-scaled (ADR-062). Each fit is kept or flagged by a uniform convergence check (R̂ < 1.05 and effective sample size > 400; Vehtari et al. 2021): a lens is shown where it converges and marked "no fit" where it does not, so SIR appears on genuinely outbreak-shaped narratives and grays out on the diffuse or multi-wave ones no single epidemic curve describes — the same way logistic and Bass gray out on shapes they cannot fit. All three lenses are now closed-form (the SIR ODE solve is gone), so a cluster a lens cannot fit is marked non-converged quickly. AICc accompanies each fit as a displayed diagnostic and selects no model.
...
- **Epidemic / diffusion models**: Kermack & McKendrick 1927; Schlickeiser & Kröger 2020 (closed-form SIR solution, *J. Phys. A* 53:505601); Verhulst 1838 (logistic); Bass 1969 (diffusion of innovations); Sultan, Farley & Lehmann 1990 (Bass meta-analysis); Gelman et al. *Bayesian Data Analysis* 3rd ed.
